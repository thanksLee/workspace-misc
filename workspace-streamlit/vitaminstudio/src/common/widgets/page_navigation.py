...
class CommonPageNavigation:
    def __init__(self, session_state_manager: SessionStateManager, total_cnt: int = 0):
        ...
        self._paging_info: PageNaviagtionDTO = self._session_state_manager.get_session_state('current_paging_info')
        self._display_cnt: int = self._session_state_manager.get_session_state('current_display_cnt')

        # 1 ~ self._pages_per_size 만큼의 데이터
        self._display_cnt = self._display_cnt
        self._page_size = (self._display_cnt * self._pages_per_size)

        # 전체 페이지 수 계산
        self._total_pages = math.ceil(self._total_cnt / self._display_cnt)
        print(f'self._total_pages : {self._total_pages}')
        ...
        if not self._paging_info:
            self._paging_info = PageNaviagtionDTO(
                page_idx=1,
                page_next=0,
                min_page=1,
                max_page=self._pages_per_size,
                display_cnt=self._display_cnt
            )
    ...
    def render(self):
        ...
        if page_next > 0:
            if cols[0].button("◀", use_container_width=True):
                ...
                self.handle_current_page(self._start_page - 1)
                st.rerun()
        ...
        for i, page_num in enumerate(range(self._start_page, self._end_page + 1)):
            ...
            if (i + 1 + (self._pages_per_size * page_next)) == current_page:
                btn_disable = True
            else:
                btn_disable = False
                print(f'b : {current_page}')
        ...
        if self._end_page < self._total_pages:
            if cols[6].button("▶", use_container_width=True):
                ...
                self.handle_current_page(self._end_page + 1)
                st.rerun()

    def handle_current_page(self, page):
        '''페이지 선택 및 출력 함수'''
        self._paging_info.page_idx = page
        self._session_state_manager.set_session_state('current_page_idx', page)
